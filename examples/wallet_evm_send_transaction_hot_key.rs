...
use lux_types::{jsonrpc::client::evm as json_client_evm, key, wallet};
...
async fn main() -> io::Result<()> {
    ...
    let max_fee_per_gas_in_lux = format_units(max_fee_per_gas, "ether").unwrap();
    let max_priority_fee_per_gas_in_gwei = wallet::evm::wei_to_gwei(max_priority_fee_per_gas);
    let max_priority_fee_per_gas_in_lux = format_units(max_priority_fee_per_gas, "ether").unwrap();

    log::info!(
        "[estimated] max_fee_per_gas: {max_fee_per_gas_in_gwei} GWEI, {max_fee_per_gas_in_lux} LUX"
    );
    log::info!("[estimated] max_priority_fee_per_gas: {max_priority_fee_per_gas_in_gwei} GWEI, {max_priority_fee_per_gas_in_lux} LUX");
