...
/// Converts "bech32::encode"d LUX address to the short address bytes (20-byte) and HRP for network name.
pub fn lux_address_to_short_bytes(chain_alias: &str, addr: &str) -> io::Result<(String, Vec<u8>)> {
...
/// RUST_LOG=debug cargo test --package lux-types --lib -- key::secp256k1::address::test_lux_address_to_short_bytes --exact --show-output
#[test]
fn test_lux_address_to_short_bytes() {
    ...
    let x_lux_addr = pubkey.to_hrp_address(1, "X").unwrap();
    let p_lux_addr = pubkey.to_hrp_address(1, "P").unwrap();
    log::info!("LUX X address: {}", x_lux_addr);
    log::info!("LUX P address: {}", p_lux_addr);

    let (hrp, parsed_short_addr) = lux_address_to_short_bytes("X", &x_lux_addr).unwrap();
    assert_eq!(hrp, "lux");
    assert_eq!(parsed_short_addr, short_addr);

    let (hrp, parsed_short_addr) = lux_address_to_short_bytes("P", &p_lux_addr).unwrap();
    assert_eq!(hrp, "lux");
    assert_eq!(parsed_short_addr, short_addr);
}
