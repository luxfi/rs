...
/// ref. "node"/vms/platformvm/signer.ProofOfPossession"
...
impl ProofOfPossession {
    ...
    /// ref. "node"/vms/platformvm/signer.ProofOfPossession.Verify"
    ...
    pub fn load_pubkey(&self) -> io::Result<public_key::Key> {
        // ref. "node/vms/platformvm/signer.ProofOfPossession.UnmarshalJSON"
        let pubkey = public_key::Key::from_bytes(&self.public_key)?;
        Ok(pubkey)
    }
}

/// RUST_LOG=debug cargo test --package lux-types --all-features --lib -- key::bls::test_proof_of_possession --exact --show-output
#[test]
