    ...
    pub async fn issue(&self) -> Result<(ids::Id, bool)> {
        ...
        if cur_balance_p < self.inner.inner.tx_fee {
            return Err(Error::Other {
                message: format!("key address {} (balance {} nano-LUX, network {}) does not have enough to cover stake amount + fee {}", self.inner.inner.p_address, cur_balance_p, self.inner.inner.network_name, self.inner.inner.tx_fee),
                retryable: false,
            });
        };
        log::info!(
