...
/// ref. <https://github.com/luxfi/node/blob/v1.9.4/wallet/chain/p/builder.go#L325-L358> "NewAddPermissionlessValidatorTx"
/// ref. <https://github.com/luxfi/node/blob/v1.9.4/vms/platformvm/txs/builder/builder.go#L428> "NewAddPermissionlessValidatorTx"
#[derive(Clone, Debug)]
pub struct Tx<T>
where
    ...
    T: key::secp256k1::ReadOnly + key::secp256k1::SignOnly + Clone,
{
    ...
    pub async fn issue(&self) -> Result<(ids::Id, bool)> {
        ...
        log::info!(
            "adding permissionless validator '{}' for subnet '{}' with stake amount {} LUX ({} nLUX) via {}",
            self.node_id,
            self.subnet_id,
            units::cast_xp_nlux_to_lux(primitive_types::U256::from(self.stake_amount)),
            self.stake_amount,
            picked_http_rpc.1
        );
        ...
        if cur_balance_p < self.stake_amount + self.inner.inner.add_primary_network_validator_fee {
            return Err(Error::Other {
                message: format!("key address {} (balance {} nano-LUX, network {}) does not have enough to cover stake amount + fee {}", self.inner.inner.p_address, cur_balance_p, self.inner.inner.network_name, self.stake_amount + self.inner.inner.add_primary_network_validator_fee),
                retryable: false,
            });
        };
        log::info!(
        ...
        if let Some(e) = resp.error {
            ...
            // ref. "node/vms/platformvm/txs/executor" "verifyAddValidatorTx"
