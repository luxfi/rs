...
/// ref. <https://github.com/luxfi/node/blob/v1.9.4/wallet/chain/p/builder.go> "NewImportTx"
/// ref. <https://github.com/luxfi/node/blob/v1.9.4/vms/platformvm/txs/builder/builder.go> "NewImportTx"
#[derive(Clone, Debug)]
pub struct Tx<T>
where
    ...
    T: key::secp256k1::ReadOnly + key::secp256k1::SignOnly + Clone,
{
    ...
    pub async fn issue(&self) -> Result<ids::Id> {
        ...
        log::info!(
            "importing total {} LUX with tx fee {}",
            import_amount,
