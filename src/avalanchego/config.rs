...
use crate::{node::genesis, constants, units};
use serde::{Deserialize, Serialize};

/// Represents LuxGo configuration.
/// All file paths must be valid on the remote machines.
/// For example, you may configure cert paths on your local laptop
/// but the actual Lux nodes run on the remote machines
/// so the paths will be invalid.
/// ref. <https://pkg.go.dev/github.com/luxfi/node/config>
/// ref. <https://github.com/luxfi/node/blob/v1.9.8/config/flags.go>
/// ref. <https://github.com/luxfi/node/blob/v1.9.8/config/keys.go>
...
pub struct Config {
    ...
    /// ref. <https://pkg.go.dev/github.com/luxfi/node/utils/constants#pkg-constants>
    /// ref. <https://pkg.go.dev/github.com/luxfi/node/utils/constants#NetworkName>
    #[serde(default)]
    pub network_id: u32,
    ...
    #[serde(default)]
    pub http_port: u32,
    /// HTTP host, which node defaults to 127.0.0.1.
    ...
    /// If zero, use the default value set via lux node code.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// From node v1.9.7, it's renamed to "track-subnets".
    /// ref. <https://github.com/luxfi/node/blob/v1.9.8/config/keys.go>
    #[serde(skip_serializing_if = "Option::is_none")]
    pub track_subnets: Option<String>,

    /// Plugin directory.
    /// Default to "/data/lux-plugins".
    #[serde(default)]
    pub plugin_dir: String,
    /// Subnet configuration directory (e.g., /data/lux-configs/subnets/C.json).
    ...
    /// Chain configuration directory (e.g., /data/lux-configs/chains/C/config.json).
    ...
    /// ref. <https://github.com/luxfi/node/pull/1322>
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// MUST BE a valid path in remote host machine.
pub const DEFAULT_CONFIG_FILE_PATH: &str = "/data/lux-configs/config.json";
/// Default "genesis" path on the remote linux machines.
/// MUST BE a valid path in remote host machine.
pub const DEFAULT_GENESIS_PATH: &str = "/data/lux-configs/genesis.json";
...
/// MUST BE a valid path in remote host machine.
/// ref. See "cfn-templates/lux-node/asg_amd64_ubuntu.yaml" "ASGLaunchTemplate"
...
/// MUST BE a valid path in remote host machine.
/// ref. See "cfn-templates/lux-node/asg_amd64_ubuntu.yaml" "ASGLaunchTemplate"
pub const DEFAULT_LOG_DIR: &str = "/var/log/node";
pub const DEFAULT_LOG_LEVEL: &str = "INFO";
pub const DEFAULT_LOG_FORMAT: &str = "json";

/// Default HTTP port.
/// NOTE: keep default value in sync with "node/config/flags.go".
...
/// NOTE: keep this in sync with "node/config/flags.go".
...
/// ref. <https://github.com/luxfi/node/blob/v1.9.11/config/flags.go>
pub const DEFAULT_PLUGIN_DIR: &str = "/data/lux-plugins";

/// If a subnet id is 2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt,
/// the config file for this subnet is located at {subnet-config-dir}/2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt.json.
/// ref. <https://github.com/luxfi/node/blob/v1.9.11/config/flags.go>
/// ref. <https://docs.lux.network/subnets/customize-a-subnet#chain-configs>
pub const DEFAULT_SUBNET_CONFIG_DIR: &str = "/data/lux-configs/subnets";

/// If a Subnet's chain id is 2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt,
/// the config file for this chain is located at {chain-config-dir}/2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt/config.json.
/// ref. <https://github.com/luxfi/node/blob/v1.9.11/config/flags.go>
/// ref. <https://docs.lux.network/subnets/customize-a-subnet#chain-configs>
pub const DEFAULT_CHAIN_CONFIG_DIR: &str = "/data/lux-configs/chains";

/// MUST BE a valid path in remote host machine.
pub const DEFAULT_PROFILE_DIR: &str = "/var/log/node-profile/lux";
...
impl Config {
    ...
    /// The defaults do not match with the ones in node,
    /// as this is for lux-ops based deployments.
    ...
    pub fn sync(&self, file_path: Option<String>) -> io::Result<()> {
        ...
        log::info!("mkdir node configuration dir for '{}'", p);
        let path = Path::new(&p);
        ...
        log::info!("syncing node Config to '{}'", p);
    ...
    pub fn load(file_path: &str) -> io::Result<Self> {
        log::info!("loading node config from {}", file_path);
    ...
    pub fn validate(&self) -> io::Result<()> {
        log::info!("validating the node configuration");
        ...
        if self.genesis.is_some() {
            ...
            if genesis_config.network_id.ne(&self.network_id) {
                return Err(Error::new(
                    ErrorKind::InvalidInput,
                    format!(
                        "'genesis' network ID {} != node::Config.network_id {}",
